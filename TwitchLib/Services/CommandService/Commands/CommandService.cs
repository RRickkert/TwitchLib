...
namespace TwitchLib.Services.CommandService.Commands
{
	public partial class CommandService : IService
    {
		...
		void IService.Install(ICommandServiceClient client)
		{
            Client = client;
            ...
            client.TwitchClient.OnMessageReceived += async (s, e) =>
            {
                ...
                {
                    if (msg.Message[0] == Config.PrefixChar.Value)
                        cmdMsg = msg.Message.Substring(1);
                }
                ...
                if (cmdMsg == null && Config.CustomPrefixHandler != null)
                {
                    ...
                    if (index >= 0)
                        cmdMsg = msg.Message.Substring(index);
                }
        ...
        internal void RemoveCommand(Command command)
        {
            ...
            {
                Client.TwitchClient.Log(ee.Message);
            }
        }
